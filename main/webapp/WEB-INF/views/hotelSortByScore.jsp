...
<%
List<SortedHotel> scoreView = (List) request.getAttribute("scoreView");
%>
...
.htimg { display: block; width: 600px; height: 280px; margin: 0px auto; }
	...
	<% for (int i = 0; i < scoreView.size(); i++) { %>
		<thead>
			<tr>
				<th colspan="6">
				<a href="/hotel/detail?hotel_no=<%=scoreView.get(i).getHotel_no()%>"><img src="/upload/<%=scoreView.get(i).getHotel_photo() %>" class="htimg" alt="이미지 아님"></a></th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td><h3><%=scoreView.get(i).getHotel_name() %></h3></td>
				<td rowspan="2"><%=scoreView.get(i).getHotel_tel() %></td>
			</tr>
			<tr>
				<td>리뷰 (<%=scoreView.get(i).getHotel_reviewCnt() %>) &nbsp;&nbsp; 별점 <%=scoreView.get(i).getHotel_score() %></td>
			</tr>
			<% } %>
		</tbody>
		<tfoot>
