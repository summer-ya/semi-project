...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<% List<SortedHotel> reviewView = (List) request.getAttribute("reviewView"); %>
...
.htimg { display: block; width: 600px; height: 280px; margin: 0px auto; }
    ...
    <table>
	<% for (int i = 0; i < reviewView.size(); i++) { %>
		<thead>
			<tr>
				<th colspan="6">
				<a href="/hotel/detail?hotel_no=<%=reviewView.get(i).getHotel_no()%>"><img src="/upload/<%=reviewView.get(i).getHotel_photo() %>" class="htimg" alt="이미지 아님"></a></th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td><h3><%=reviewView.get(i).getHotel_name() %></h3></td>
				<td rowspan="2"><%=reviewView.get(i).getHotel_tel() %></td>
			</tr>
			<tr>
				<td>리뷰 (<%=reviewView.get(i).getHotel_reviewCnt() %>) &nbsp;&nbsp; 별점 <%=reviewView.get(i).getHotel_score() %></td>
			</tr>
			<% } %>
		</tbody>
		<tfoot>
